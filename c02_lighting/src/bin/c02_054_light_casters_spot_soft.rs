...
pub fn main() {
    ...
    let (lighting_shader, cubevao, diffuse_map, specular_map, cube_positions) = {
        ...
        let lighting_shader =
            Shader::new(assets!("shaders/5.4.light_casters.vs"), assets!("shaders/5.4.light_casters.fs"));
        // let lamp_shader = Shader::new(assets!("shaders/5.4.lamp.vs"), assets!("shaders/5.4.lamp.fs"));
        ...
        (lighting_shader, cubevao, diffuse_map, specular_map, cube_positions)
    };
    ...
    event_loop.run(move |event, _, control_flow| match event {
        ...
        Event::MainEventsCleared => {
            ...
            lighting_shader.use_program();
            lighting_shader.set_vector3("light.position", &camera.position.to_vec());
            lighting_shader.set_vector3("light.direction", &camera.front);
            lighting_shader.set_float("light.cutOff", 12.5f32.to_radians().cos());
            lighting_shader.set_float("light.outerCutOff", 17.5f32.to_radians().cos());
